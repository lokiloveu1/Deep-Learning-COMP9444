...
def my_perceptron(x):
    ...
    w = tf.get_variable('w0',[x],dtype=tf.float32,initializer=tf.constant_initializer(1))
    out0 = tf.tensordot(i, w, 1)
    out = my_relu(out0)
    return i, out
...
def onelayer(X, Y, layersize=10):
    ...
    """
    w = tf.get_variable(str(random.random()),[X.get_shape()[1],layersize],dtype=tf.float32,initializer=tf.constant_initializer(1))
    b = tf.get_variable(str(random.random()), [1, layersize], dtype=tf.float32,initializer=tf.constant_initializer(0))
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    b1 = tf.get_variable(str(random.random()), [1, hiddensize], dtype=tf.float32, initializer=tf.constant_initializer(1))
    ...
    b2 = tf.get_variable(str(random.random()), [1, outputsize], dtype=tf.float32, initializer=tf.constant_initializer(1))
    hidden_layer_out = tf.add(tf.matmul(X, w1),b1)
    hidden_layer_out_relu = my_relu(hidden_layer_out)
    logits = tf.add(tf.matmul(hidden_layer_out_relu, w2),b2)
    preds = tf.nn.softmax(logits)
    batch_xentropy = tf.nn.softmax_cross_entropy_with_logits(labels=Y, logits=logits)
    batch_loss = tf.reduce_mean(batch_xentropy)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    conv1 = tf.layers.conv2d(inputs=X, filters=convlayer_sizes[0],activation=my_relu, kernel_size=filter_shape[0], padding=padding)
    conv2 = tf.layers.conv2d(inputs=conv1, filters=convlayer_sizes[1], activation=my_relu,kernel_size=filter_shape[1], padding=padding)
    convolution = tf.reshape(conv2, [tf.shape(conv2)[0], 7840])
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(convolution, Y, outputsize)
